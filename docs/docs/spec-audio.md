...
As with visual encodings, each value is a field reference that inherits the field's definition, and may locally override `aggregate`, `timeUnit`, `scale`, or `sort` (audio encodings can't `bin`). The `"None"` sentinel removes an inherited transform.
...
Each step of the traversal is a group of data points; the encoded fields are evaluated within that group (this is where an `aggregate` on the encoding matters). Traversal fields may be coarsened with `bin` (quantitative) or `timeUnit` (temporal), but can't be aggregated — they define the steps, not the measures.
